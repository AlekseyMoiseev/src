...
public class HeaderDataExtractor {
    	 ...
    	 */
    	private static Logger logger = Logger.getLogger(HeaderDataExtractor.class.getName());
	...
	return result;
    	...
    	private String[] extractFile(DicomFile dfile, boolean extendedInfo) {
    		...
    		try {
    			...
    			int i = 0;
    			if (dh == null ) {
    			    logger.warn("NO DICOM file (Header is NULL): " + dfile.getFile().getAbsolutePath());
    				return null;
    			}
    			for (HeaderTag ht : headerTags) {
    			    	...
    			    	try {
							...
							logger.log(Level.WARN, "Error getting Tag: " + ht.toString() + " on file "+ dfile.getFile().getName(), e);
						}
    			...
    			if (isEmpty) {
    			    logger.warn("NO DICOM file: " + dfile.getFile().getAbsolutePath());
    				return null;
    			}
    			if (extendedInfo) {
