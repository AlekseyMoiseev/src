...
public class FILEChooserDicom extends JPanel implements ActionListener,
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals(JFileChooser.CANCEL_SELECTION)) {
			...
			selectFiles(this.fileChooser.getSelectedFiles());
	...
	private void selectFiles(File[] fileArr) {
		...
		if (fileArr.length == 1 && f.isFile()) {
			...
			for (File file : fileArr) {
				if (file.isDirectory()) {
					visitAllFiles(file, al);
				} else {
					al.add(file);
				}
			}
			Collections.sort(al);
			File[] selectedFiles = al.toArray(new File[al.size()]);
			filePanel.getDicomFileDialog().setSelectedFiles(selectedFiles);
		}
...
//		if (f.isFile()) {
//			File[] files = new File[]f.getParentFile().listFiles();
//			Arrays.sort(files);
//			filePanel.getDicomFileDialog().setSelectedFiles(files);
//		} else {
//			File[] files = getFilesRecursive(f);
