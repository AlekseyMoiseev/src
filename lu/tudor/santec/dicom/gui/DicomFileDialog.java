...
public class DicomFileDialog extends JDialog implements SettingListener {
	 ...
	 */
	private static Logger logger = Logger.getLogger(DicomFileDialog.class.getName());
...
//		// add dicomdir button
	...
	public boolean isFoldersSelected() {
		return this.foldersSelected;
	}

	public void setFoldersSelected(boolean foldersSelected) {
...
//	}
	...
	public JButton getToolBarButton() {
		return new JButton(DicomIcons.getIcon32(DicomIcons.PACS));
	}
	...
	private void showQueryPanel() {
		...
		if (enabled) {
			...
			try {
				openOnReceive = ((Boolean) dicomSettingsPlugin.getValue(DICOMSettingsPlugin.QUERY_OPENONRECEIVE)).booleanValue();
			} catch (Exception e) {}
			
			dicomQueryPanel = new QueryPanel(dcmUrl, this, openOnReceive);
...
//					.getValue(DICOMSettingsPlugin.REC_DIR)));
			...
			try {
				...
				DcmURL url = new DcmURL("dicom://" + 
						this.dicomSettingsPlugin.getValue(DICOMSettingsPlugin.REC_AET) + "@" +
				...
				JOptionPane.showMessageDialog(this,
					Translatrix.getTranslationString("dicom.StoreStartError", params)+"\n"+e.getMessage(),
				    "DICOM STORE ERROR",
				    JOptionPane.ERROR_MESSAGE);
				logger.warn(Translatrix.getTranslationString("dicom.StoreStartError", params)+"\n"+e.getMessage(), e);
			}
		}
	}
...
//					return dicomDirReader.getImageFromSeriesUID(parts[0],Integer.parseInt(parts[1]));			
				...
				logger.warn("dicomdir: " + dicomdir + " does not exist");
			}
		}
	...
	public String saveFiletoLocalPacs(File file) throws Exception {
		try {
			...
			logger.warn("unable to save image to: " + this.createLocalDicomSender().getUrl());
		}
