	...
	public Series(File[] imageFiles, boolean inMemory, boolean openViaImageJ) {
		...
		for (int i = 0; i < imageFiles.length; i++) {
			...
			try {
				...
				imagePlus = DicomOpener.loadImageStack(imageFiles[i], null, null, openViaImageJ);
...
//						System.out.println("adding Image: " + imageName);
				...
				logger.log(Level.WARN, "File has no dicomheader", e);
			}
		}
...
//				imagePlus = new ImagePlus(f.getAbsolutePath());
	...
	public void setSlice(int sliceNum) {
		try {
			currentImage = sliceNum;
			String f = this.seriesImageFiles.get(sliceNum);
			imagePlus = this.seriesImages.get(f);
			double min = 0, max = 0;
			try {
				max = imagePlus.getProcessor().getMax();
				min = imagePlus.getProcessor().getMin();
			...
			if (imagePlus == null) {
				logger.log(Level.INFO, "Loading from file instead of memory");
			}
