...
import lu.tudor.santec.dicom.gui.utils.RoiCoder;
...
public class DicomImagePanel extends JTextField implements MouseListener,
	 ...
	 */
	private static Logger logger = Logger.getLogger(DicomImagePanel.class.getName());
...
//	 fields that contain helper values for painting ########################
	...
	private static Color roiColor = Color.YELLOW;

	private static int roiStrokeWidth = 1;
...
//	 fields that contain java components #############################
	...
	private LinkedHashMap<AbstractButton, ImageListener> secondaryModeButtons = new  LinkedHashMap<AbstractButton, ImageListener>();
	private LinkedHashMap<AbstractButton, ImageListener> secondaryFunctionButtons = new  LinkedHashMap<AbstractButton, ImageListener>();
	...
	private void createImageArea(int width, int height,
		...
		this.actionShowSR = new AbstractAction(Translatrix.getTranslationString("dicom.dicomImagePanel.showSR"), DicomIcons.getScreenDependentIcon(DicomIcons.ACTION_SR)) {
			private static final long serialVersionUID = 1L;
			private SRDialog srDialog;
			public void actionPerformed(ActionEvent e) {
				if (srDialog == null) {
					srDialog = new SRDialog();
				}
				File f = new File(image.getOriginalFileInfo().directory + image.getOriginalFileInfo().fileName);
				srDialog.showReports(new File[] {f});
			}
		};
		...
		this.actionHelp = new AbstractAction(Translatrix.getTranslationString("dicom.dicomImagePanel.help"), DicomIcons.getScreenDependentIcon(DicomIcons.ACTION_HELP)) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				comps = secondaryToolBar.getComponents();
				for (int i = 0; i < comps.length; i++) {
					try {
						AbstractButton element = (AbstractButton) comps[i];
						jp.add(new JLabel(element.getToolTipText(),element.getIcon(), JLabel.LEFT));
	...
	public void setImage(ImagePlus image) {
		...
		if (this.imageProcessor == null) {
			logger.warn("This Object is no Image");
			return;
		}
		...
		try {
			...
			try {
        		...
        		actionShowSR.setEnabled(modality.equals("SR"));
        		...
        		try {
					if (dicomHeader.containsHeaderTag(RoiCoder.TAG_ROIS)) {
						Vector<Roi> rois = RoiCoder.decodeRois(dicomHeader.getDicomObject().getBytes(RoiCoder.TAG_ROIS));
						setRois(rois);
					}
					
					if (dicomHeader.containsHeaderTag(RoiCoder.TAG_TEXTS)) {
						Vector<OSDText> texts = RoiCoder.decodeTexts(dicomHeader.getDicomObject().getBytes(RoiCoder.TAG_TEXTS));
						setOSDTexts(texts);
					}
	...
	public void setRoiStrokeWidth(int strokeWidth) {
		roiStrokeWidth = strokeWidth;
	}
...
//			e.printStackTrace();
	...
	private static void drawRois(Graphics g, Vector<Roi> rois, int offsetX,
		...
		BasicStroke stroke = new BasicStroke(roiStrokeWidth);
		...
		for (Iterator<Roi> iter = rois.iterator(); iter.hasNext();) {
			...
			try {
				...
				g2.setColor(new Color(Integer.parseInt(colors[0].trim()),
						Integer.parseInt(colors[1].trim()), Integer
								.parseInt(colors[2].trim())));
			} catch (Exception e) {
				if (roi.getStrokeColor() != null) 
					g2.setColor(roi.getStrokeColor());
				else
					g2.setColor(roiColor );
			}
			if (roi instanceof Line) {
				Line line = (Line) roi;
				g2.drawLine((int) (line.x1 * factorX) + offsetX,
						(int) (line.y1 * factorY) + offsetY,
						(int) (line.x2 * factorX) + offsetX,
						(int) (line.y2 * factorY) + offsetY);
				
			} else if (roi instanceof PolygonRoi && roi.getType() == PolygonRoi.FREELINE ) {
				...
				g2.draw(path);
			} else if (roi instanceof OvalRoi || roi instanceof Roi) {
				Polygon p = roi.getPolygon();
				for (int i = 0; i < p.xpoints.length; i++) {
					p.xpoints[i] = (int) (p.xpoints[i] * factorX) + offsetX;
				}
...
//		System.out.println("DEFAULT Cursor");
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.isPopupTrigger()) {
			...
			} else if (this.secondaryModeButtons.containsKey(currentButton)){
				((ImageListener) this.secondaryModeButtons.get(currentButton)).pixelClicked(image,xPixel, yPixel);
			}
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			if ((e.isControlDown() || currentButton.equals(buttonMove)) && moveable) {
				...
				((ImageListener) this.secondaryModeButtons.get(currentButton)).mouseMoved(xPixel, yPixel, e);
			} else {
	...
	public void keyTyped(KeyEvent e) {
	    ...
	    if (e.getKeyChar() == 'r' || e.getKeyChar() == 'R') {
			...
			KeyListener[] keyListeners = currentButton.getKeyListeners();
	...
	public void showBigArea(boolean show) {
		if (this.bigArea == null) {
			...
			if (this.bigArea.createSocondaryToolBar() != null) {
				this.bigDialog.getContentPane().add(this.bigArea.createSocondaryToolBar(), BorderLayout.EAST);
			}
			this.bigArea.setImage(this.image);
...
//				System.out.println(tmp + "+" +(tmp*imageAspect));
	...
	public DicomImagePanel createBigArea() {
		...
		this.bigArea.secondaryModeButtons = this.secondaryModeButtons;
		this.bigArea.secondaryFunctionButtons = this.secondaryFunctionButtons;
...
//	}
	...
	public JToolBar createSocondaryToolBar() {
		
		if (secondaryToolBar != null) {
			return secondaryToolBar;
		}
		
		if (secondaryFunctionButtons.size() == 0 && secondaryModeButtons.size() == 0) {
			return null;
		}
		
		secondaryToolBar = new JToolBar(JToolBar.VERTICAL); 
		secondaryToolBar.setMinimumSize(new Dimension(20,20));
		secondaryToolBar.addKeyListener(this);
		
		for (Iterator<AbstractButton> iter = secondaryModeButtons.keySet().iterator(); iter.hasNext();) {
			JToggleButton element = (JToggleButton) iter.next();
			secondaryToolBar.add(element);
		}
		
		secondaryToolBar.addSeparator();
		
		for (Iterator<AbstractButton> iter1 = secondaryFunctionButtons.keySet().iterator(); iter1.hasNext();) {
	...
	public JToolBar createToolBar() {
		...
		if (useDefaultButtons) {
    		...
    		this.actionShowSR.putValue(Action.SHORT_DESCRIPTION, Translatrix.getTranslationString("dicom.dicomImagePanel.showSR"));
    		toolBar.add(this.actionShowSR);
	...
	private void setModeButton(JToggleButton jtb) {
		setModeButton(jtb, false);
	}
	
	private void setModeButton(JToggleButton jtb, boolean isSecondary) {
			...
			try {
				...
				try {
					...
					if (!isSecondary) {
						...
						for (Iterator<ImageListener> it = this.secondaryModeButtons.values().iterator(); it.hasNext();) {
							((ImageListener) it.next()).optionSelected(false);
						}
						for (Iterator<AbstractButton> iter = this.secondaryModeButtons.keySet().iterator(); iter.hasNext();) {
							((JToggleButton) iter.next()).setSelected(false);
						}
					} else {
						((ImageListener) this.secondaryModeButtons.get(element)).optionSelected(isButton);
						for (Iterator<ImageListener> it = this.modeButtons.values().iterator(); it.hasNext();) {
							((ImageListener) it.next()).optionSelected(false);
						}
						for (Iterator<AbstractButton> iter = this.modeButtons.keySet().iterator(); iter.hasNext();) {
							((JToggleButton) iter.next()).setSelected(false);
						}
					}
				} catch (Exception e) {
	...
	public JToggleButton addSecondaryModeButton(ImageIcon icon, String text, ImageListener il) {
		JToggleButton b = new JToggleButton(new AbstractAction("",icon) {
			private static final long serialVersionUID = 1L;
			public void actionPerformed(ActionEvent e) {
				if (bigArea != null) {
					bigArea.setModeButton((JToggleButton) e.getSource(), true);
				}
			
			}
		});
		b.setToolTipText(text);
		this.secondaryModeButtons.put(b,il);
		return b;
	}
	...
	public JToggleButton addModeButton(ImageIcon icon, String text, ImageListener il) {
		JToggleButton b = new JToggleButton(new AbstractAction("",icon) {
			...
			public void actionPerformed(ActionEvent e) {
				if (bigArea != null) {
					bigArea.setModeButton((JToggleButton) e.getSource(), false);
				}
	...
	public void addSecondaryFunctionButton(AbstractButton button, ImageListener il) {
		this.secondaryFunctionButtons.put(button,il);
	}
	
	
	//------ Roilistener is fired on updates on the Default or normal Rois
	...
	public synchronized void fireRoiEvent( String roiEventname ) {
		for( RoiListener l : listeners.getListeners( RoiListener.class )) {
