...
public class DicomMatcher {
	 ...
	 */
	private static Logger logger = Logger.getLogger(DicomMatcher.class.getName());
	...
	public void setHeaderTags(HeaderTag[] tags) {
		...
		logger.info("Setting Filter Tags: " +Arrays.toString(this.headerTags));
	}
	
	public void setHeaderTags(List<HeaderTag> tags) {
		setHeaderTags(tags.toArray(new HeaderTag[0]));
	}
	...
	public boolean matchFile(DicomHeader dh) {
	    if (headerTags == null) {
			...
			logger.warn("no filterTags set -> MATCH");
			return true;
	    }
			...
			for (int i = 0; i < headerTags.length; i++) {
			    try {
					...
					if (tagValue == null || "".trim().equals(tagValue)) {
