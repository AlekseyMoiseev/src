...
//				.getAbsolutePath(), false, "password");
		...
		this.ddr = pacsPanel.getDicomDirReader();
		
		this.add(pacsPanel, cc.xyw(1, 4, 3));
		
//		this.parent.settoolBarButton(Translatrix.getTranslationString("dicom.ShowOpenDialog"));
	...
	public void setPath(File f) {
		...
		this.dicomStorageServer.setDicomDirReader(pacsPanel.getDicomDirReader());
	}
...
//		this.parent.settoolBarButtonIcon(DicomIcons.getIcon(DicomIcons.PACS));
		switch (status) {
		case SERVER_RESET:
...
//			this.parent.toolBarButton.setIcon(Icons.getIcon(Icons.PACS_OFF));
	...
	public void startDicomServer(String AET, Integer port) throws Exception {
		...
		dicomStorageServer = new DicomStorageServer(AET, port, file
				.getAbsolutePath(),false, "password", false, this.ddr);
...
//			this.setPath(file);
	...
	public void setDicomFilterTags(@SuppressWarnings("rawtypes") Vector headerTags) {
		pacsPanel.setDicomFilterTags(headerTags);
	}
	
	public void addDicomListener(DICOMListener dl) {
		dicomStorageServer.addDICOMListener(dl);
	}
	
	public void removeDicomListener(DICOMListener dl) {
		dicomStorageServer.removeDICOMListener(dl);
	}
	...
	public void deleteRecord(String level, DicomHeader dh) throws Exception{
		ddr.deleteRecord(level, dh);
	}
	
    public final void setUseDicomDir(boolean useDicomDir) {
    	dicomStorageServer.setUseDicomDir(useDicomDir);
    }
