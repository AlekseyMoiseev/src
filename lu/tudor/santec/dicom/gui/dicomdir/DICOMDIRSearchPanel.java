...
public class DICOMDIRSearchPanel extends JPanel implements ActionListener,
		ListSelectionListener, MouseListener, DicomFilter, PropertyChangeListener {
...
//			this.dicomImagePreview.loadImage(null);
			...
			ddr.loadDicomDirFile(inFile, create, canDelete);
...
//		        // for one or more series
	...
	public void send() {
		...
		// for Series
		    	...
		    	for (int i = 0; i < rows.length; i++) {
		    	    ...
		    	    logger.warn("sending series with " + fArr.length + " images");
		...
		// for study
		    	...
		    	for (int i = 0; i < rows.length; i++) {
		    	    ...
		    	    logger.warn("sending studies with " + sendFiles.size() + " images");			    
			}
		...
		// for patient
				...
				for (Iterator iter2 = series.iterator(); iter2.hasNext();) {
        	    	...
        	    	logger.warn("sending patient with " + sendFiles.size() + " images");			    				
		}
	...
	private void selectPatient() {
		try {
			...
			logger.debug("error updating dicom view");
			this.parent.getParentDialog().setWaitCursor(false);
		}
	}
	...
	private void selectStudy() {
		try {
			...
			logger.info("error updating dicom view");
			this.parent.getParentDialog().setWaitCursor(false);
		}
	}
...
//	             	deleteButton.setEnabled(hasSelection);
	...
	public void reIndex() {
		...
		if (! this.create) {
			logger.warn("not reIndexing DICOMDIR, not writeable");
		}
		...
		try {
			...
			final ProgressMonitor progressMonitor = new ProgressMonitor(this, Translatrix.getTranslationString("dicom.reindexing"), "", 0, files.length - 1);

			new Thread() {
				public void run() {
					logger.info("Starting reIndexing of DICOMDIR");
					for (int i = 0; i < files.length; i++) {
						if (!files[i].isDirectory())
							try {
								...
								logger.warn("Error reIndexing DICOMDIR", e);
							}
					}

					progressMonitor.close();
					logger.info("reIndexing of DICOMDIR finished");
			...
			logger.warn("error reindexing dicom view");
			e1.printStackTrace();
		}
	...
	public void updateView() {
		Vector patients = null;
		if (ddr.isOK()) {
			try {
				patients = ddr.getPatients();
			} catch (Exception e) {
		...
		patientTableBorder.setTitle(Translatrix.getTranslationString("dicom.Patients") + ": [" + patientTableModel.getRowCount() +"]");
		studyTableModel.setStudies(new Vector());
		seriesTableModel.setSeries(new Vector());
		pictureTableModel.setPictures(new Vector());
		dicomImagePreview.loadImage(null);
	}

	public void open() {
		if (ddr != null && ! ddr.isWriteable()) {
			logger.info("Reloading dicomdir");
			reload(inFile);
		}
	}
