...
//	private TitledBorder pictureTableBorder;
	...
	private static Logger logger = Logger.getLogger("lu.tudor.santec.dicom.gui.DicomDirSearchPanel");
...
//				"6dlu, fill:80dlu:grow, 4dlu, 80dlu, 4dlu, pref, 6dlu");
		...
		FormLayout fl = new FormLayout(
				"6dlu, fill:100dlu:grow, 6dlu",
				"6dlu, fill:80dlu:grow, 4dlu, 80dlu, 4dlu, 80dlu, 4dlu, pref, 6dlu");
		...
		patientTable.getColumnModel().getColumn(3).setMaxWidth(30);
...
//		this.add(jsp1, cc.xyw(4, 2, 3));
		this.add(jsp1, cc.xyw(2, 4, 1));
		studyTable.getColumnModel().getColumn(3).setMaxWidth(100);
		studyTable.getColumnModel().getColumn(4).setMaxWidth(90);
		...
		seriesTable.getColumnModel().getColumn(0).setMaxWidth(40);
		seriesTable.getColumnModel().getColumn(1).setMaxWidth(60);
		seriesTable.getColumnModel().getColumn(4).setMaxWidth(40);
...
//		this.add(jsp3, cc.xy(6, 4));
		...
		openButton = new JCheckBox(Translatrix.getTranslationString("dicom.OpenOnReceive"));
		openButton.setSelected(openOnReceive);
		openButton.addActionListener(this);
		bbuilder.addFixed(openButton);
...
//		this.add(bbuilder.getPanel(), cc.xyw(2, 6, 5));
		this.add(bbuilder.getPanel(), cc.xyw(2, 8, 1));
...
//		this.reload("*");
	...
	public void query(Vector<HeaderTag> filters, DcmQR.QueryRetrieveLevel level) {
		...
		try {
			if (firstrun) {
				String retVal = DICOMSettingsPlugin.testPACS("PACS", this, this.url);
				if (retVal != null) {
					logger.warn("Error connecting to PACS " + retVal);
			...
			logger.info("Query for " + level + " with keys:\n" + filters);
...
//			this.pictureTableBorder.setTitle(Translatrix.getTranslationString("dicom.Pictures")+": [" + this.pictureTableModel.getRowCount()+ "]");
			...
			logger.warn("error updating dicom view");
			this.parent.getParentDialog().setWaitCursor(false);
		}
//		sendButton.setEnabled(false);
		movelocalButton.setEnabled(false);
	...
	public static List<DcmQR.QueryRetrieveLevel> getQueryLevels() {
		List<DcmQR.QueryRetrieveLevel> keys = new ArrayList<DcmQR.QueryRetrieveLevel>();
	...
	public static List<HeaderTag> getQueryFields(DcmQR.QueryRetrieveLevel level) {
		List<HeaderTag> keys = new ArrayList<HeaderTag>();
		if (level.equals(DcmQR.QueryRetrieveLevel.PATIENT)) {
...
//			this.moveTo(null);
			...
			if (this.parent.getParentDialog().pacsPanel == null) {
				JOptionPane.showMessageDialog(this,
					    "Local DICOM Store is not activated",
					    "MOVE Error",
					    JOptionPane.ERROR_MESSAGE);

				logger.warn("Local DICOM Store is not activated");
				return;
			}
			
			
			DcmURL url = new DcmURL("dicom://" + 
					this.parent.getParentDialog().pacsPanel.AET + "@" +
					DICOMSettingsPlugin.getIP() + ":" +
					this.parent.getParentDialog().pacsPanel.port);
...
//				} else	
				...
				if (seriesTable.getSelectedRow() != -1) {
					try {
						...
						for (int i = 0; i < rows.length; i++) {
						    DicomObject series = seriesTableModel.getDimse(seriesTableSorter.modelIndex(rows[i]));
							dq.moveSeries(QuerySearchPanel.this, moveAET, series);
						}
					} catch (Exception e) {
						ErrorDialog.showErrorDialog(parent.getParentDialog(), "Error on DICOM MOVE" , 
								e.getLocalizedMessage(), e);
...
//						}
						...
						int[] rows = studyTable.getSelectedRows();
						for (int i = 0; i < rows.length; i++) {
						    DicomObject series = studyTableModel.getDimse(studyTableSorter.modelIndex(rows[i]));
							dq.moveStudies(QuerySearchPanel.this, moveAET, series);
						}
