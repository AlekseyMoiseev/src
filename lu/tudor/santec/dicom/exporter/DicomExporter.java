        ...
        if (!isStack) {
        	...
        	if (multislice)
                ...
                for (int i = 1; i<=stackSize; i++) {
                    ...
                    header.putString(Tag.SOPClassUID, VR.UI, "1.2.840.10008.5.1.4.1.1.4"); //Single Slice instead Milti-Slice
                    int tagAsInt; //Adds necessary tags for Eclipse TPS
					try {
						tagAsInt = DicomHeader.toTagInt("0018,0020");
						if (header.getString(tagAsInt)==null) {
							header.putString(tagAsInt, VR.CS, "RM");	
						}	
						tagAsInt = DicomHeader.toTagInt("0018,0021");
						if (header.getString(tagAsInt)==null) {
							header.putString(tagAsInt, VR.CS, "NONE");	
						}	
						tagAsInt = DicomHeader.toTagInt("0008,0008");
						header.putString(tagAsInt, VR.CS, "DERIVED\\SECONDARY");
    ...
    public void writeMultiframe(DicomObject header, ImagePlus ip, File dcmFile) throws IOException {
        ...
        try {
            ...
            for (int i = 1; i<=stackSize; i++) {
                ...
                sliceFI.intelByteOrder=true; //Little Endian 
            ...
            for (int i=0; i<result.length; i++) {
                try{
                    ...
                    if(!tagID.startsWith("0028")) {
                        ...
                        if (tagID.equals("0028,1040")) {
                                dcmObj.putString(tagAsInt, VR.CS, tagValue);
                            }
                        else {
                            ht0028.put(tagID, tagValue);
                        }
                    	//needed for MR images                         
                        if (tagID.equals("0028,0030")) {
                                dcmObj.putString(tagAsInt, VR.LO, tagValue);
                        }
                        else {
                            ht0028.put(tagID, tagValue);
                        }
