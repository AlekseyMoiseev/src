...
public class DicomOpener{
	...
	/**
	 * @param p_File the file to load
	 * @param dh the dicom header of the file (if already read, else null) 
	 * @return
	 * @throws FileNotFoundException
	 */
	 ...
	 */
      ...
      /**
   	...
   	public static ImagePlus loadImage(File p_File, DicomHeader dh, Component parentComponent) throws FileNotFoundException {
   		return loadImage(p_File, dh, parentComponent, false);
   	}
	...
	public static ImagePlus loadImage(File p_File, DicomHeader dh, Component parentComponent, boolean tryOpenViaImageJFirst) throws FileNotFoundException {
		...
		if (fileSize > freeMem) {
			logger.error("File is bigger than available Memory -> NOT OPENING");
			return createDummyImage(p_File, dh, "File is bigger than available Memory -> NOT OPENING") ;
		}
		...
		try {
		    	...
		    	if (dh.hasPixelData()) {
		    		...
		    		if (tryOpenViaImageJFirst && ! dh.isCompressedImage()) {
		    			logger.debug("opening image via ImageJ");
		...
		//		    		ImageProcessor ip = Util.openProcessors(dicomReader, 0)[0];
					    	...
					    	logger.warn("Opening file via Loci-Tools failed");
					    	logger.info("Trying opening image via ImageJ");
							image = new ImagePlus(p_File.getAbsolutePath());
					    } finally {
							try {
								if (dicomReader != null)
		    		...
		    		if (image == null || image.getHeight() == 0) {
		    			return createDummyImage(p_File, dh, "Unable to open Image Data.") ;
		    		}

		    	    logger.debug("loading image took: " + (System.currentTimeMillis()-start) + "ms");
	...
	private static void setCalibration(ImagePlus image, DicomHeader dh) {
    	...
    	if (image.getFileInfo().fileType==FileInfo.GRAY16_SIGNED) {
...
//    		}
	...
	public static ImagePlus loadImageStack(File p_File, DicomHeader dh, Component parentComponent) throws FileNotFoundException {
		return loadImageStack(p_File, dh, parentComponent, false);
	}
	...
	public static ImagePlus loadImageStack(File p_File, DicomHeader dh, Component parentComponent, boolean tryOpenViaImageJFirst) throws FileNotFoundException {
		...
		//		    		ImageProcessor ip = Util.openProcessors(dicomReader, 0)[0];
				    		...
				    		if (slices >1) {
				    			// multislice
				    			logger.debug("Loading Multislice DICOM File with " + slices + " slices");
				    			...
				    			for (int i = 0; i < slices; i++) {
				    				...
				    				logger.debug("Loading Slice: " + i + "/" + slices +  " Processoranz: "  + processor.length);
		    	    ...
		    	    try {
						...
						logger.warn("error setting image calibration for image: " + p_File);
					}
		    	    image.setFileInfo(fi);			
		    	} else {
		    	    image =  createDummyImage(p_File, dh, "File does not contain Image Data.") ;
			...
			logger.error("Unable to open image file: "+ p_File.getAbsolutePath() + "\n" + e.getMessage(), e);
			if (parentComponent != null) {
			    JOptionPane.showMessageDialog(parentComponent,
				    "Unable to open image file: "
					+ p_File.getAbsolutePath() + "\n" + e.getMessage(),
